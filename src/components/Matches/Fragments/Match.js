...
class MatchComponent extends React.Component {
  ...
  apply() {
   ...
   updateMatch(searchId, searchType, currentMatch) {
     this.props.matchesUpdate(searchId, searchType, currentMatch)
     .then((response) => {
       if (response.payload && response.payload.status === 200) {
           this.props.addNotification(strings.saveMatchNotify, 'success', 'tc');
            return this.props.fetchSearches().then(action =>
    ...
    saveMatches(searchId, searchType, currentMatch) {
     this.props.saveMatch(searchId, searchType, currentMatch)
     .then((response) => {
       if (response.payload && response.payload.status === 200) {
...
currentorganizationType: profiles.organizationType,
  ...
  render() {
    ...
    const matchadminStatus = this.props.emailId.includes('digitalpartners.io') ? 'matchResult' : 'matchVerified';
    return (
      <Grid>
        {currentMatch.matchStatus === 'matchResult' &&
          <Grid.Row>
              ...
              <Grid centered>
                <Grid.Row className="no-vertical-padding">
                  <Grid.Column computer={12}>
                    ...
                    {this.props.emailId.includes('digitalpartners.io') && <div className="match-summary--title">
                  ...
                  <Grid.Column tablet={1} computer={1} className={'mobile hidden'}>
