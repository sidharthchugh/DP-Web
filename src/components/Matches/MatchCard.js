...
import {fetchMatches, matchRefresh} from '../../actions/matches';
...
render() {
  ...
  const {savedSearches, matchId, emailId} = this.props;
  // Limit Matching Profiles upto 20
  const limitMatches = matchingProfiles.slice(0, 20);
  const matches = search.matches;
  let verifiedsortMatch;
  const verifiedMatch = matches.filter(match => match.matchStatus === 'matchResult');
  console.log()
  if (emailId.includes('digitalpartners.io')) {
  verifiedsortMatch = matches;
  } else {
  verifiedsortMatch = verifiedMatch.sort((a, b) => {
  ...
  const sortedVerifiedMatches = sort(limitMatches, verifiedsortMatch);
  ...
  const matchVerified = search.matches.filter(match => match.matchStatus === 'matchResult');
      ...
      <Grid.Row>
        ...
        <div>
          <Scrollchor to={`${searchType}${this.props.index}`} animate={{offset: -50, duration: 500}}>
            <h3 className="margin-header3">{`Matches - ${searchName(this.props.userLanguage)}`}</h3>
          </Scrollchor>
        </div>
        }
        ...
        <Grid.Column computer={12}>
          ...
          <Grid className="match-card--container card-generic" id={`${searchType}${this.props.index}`}>
            <Grid.Column computer={12} className="no-vertical-padding">
              <div>
                {(matchVerified.length > 0 || matchSaved.length > 0) && <Button
                      className="button-small matchRefresh"
                      onClick={() => this.refreshMatches(search)}>
                  {'Refresh'}
                </Button>}
                <CollapsibleCardWrapper title={`Matches - ${searchName(this.props.userLanguage)}`}>
                  ...
                  {!this.props.emailId.includes('digitalpartners.io') && matchVerified.length < 1 && matchSaved.length > 0 &&
                <Grid>
                      <Grid.Row>
                        <Grid.Column computer={12}>
                        <div className="no-matchResult">
                  ...
                  {!this.props.emailId.includes('digitalpartners.io') && matchVerified.length < 1 && matchSaved.length < 1 &&
                    <Grid>
                      <Grid.Row>
                        <Grid.Column computer={12}>
                        <div className="no-matchResult">
                  ...
                  <Grid>
                    <Grid.Row>
                      <Grid.Column computer={12}>
                        {!matchId && sortedVerifiedMatches.map((matchingProfile, i) =>
                          <Match
                            key={'match-summary-' + i}
                            searchId={search._id}
...
export default connect(mapStateToProps, {fetchMatches, fetchSearchSignup, fetchSearches, matchRefresh})(MatchCard);
