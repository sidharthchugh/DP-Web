...
import CompStatus from '../Sections/CompStatus';
      ...
      <Grid id={fullprofile ? 'profile-company-description fullprofile-noheader' : 'profile-company-description'}>
        <Grid.Column computer={12} className="no-gutter">
          <form onSubmit={handleSubmit}>
            <div id={'my-Company'}className={'profile-card company-info' + (sections.company.editable ? ' edit-mode' : '')}>
              {!fullprofile && <a className="edit-button" onClick={() => this.toggleEdit('company')}>
                <img src={editIcon} role="presentation" /> {strings.edit}
              </a>}
              <div className="save-cancel-button save-cancel-button-company comp-cancel-save-edit">
