...
import {fetchProfilesView, updateProjects, toggleProjectsEdit} from 'actions/projects';
...
class ProjectViewCard extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    const {cmsData, projectId} = this.props;
      this.props.fetchProfilesView({projectId});
       if (!cmsData.cmsData) {
  ...
  // Send Values To Database
    ...
    updateProjects({updatedValue, addedProject, projectLink, projectMatchId: this.props.projectId})
    .then((response) => {
...
export default connect(mapStateToProps, { fetchProfilesView, updateProjects, addNotification, cmsDetail, toggleProjectsEdit})(ProjectViewCard);
