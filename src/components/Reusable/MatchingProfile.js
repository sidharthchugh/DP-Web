...
class MatchingProfile extends Component {
 ...
 handleFullProfileClose() {
  ...
  render() {
    const {profileId,userLanguage, searchType, user,companyName} = this.props;
    return (
    <Modal
      trigger={<h2 className="heading3 title-project matchTitleMoveDown" onClick={this.handleFullProfileOpen}>{companyName}</h2>} open={this.state.fullprofileOpen}
      onClose={this.handleFullProfileClose} closeIcon="close">
        ...
        <Modal.Actions>
          <Button  className="button-small cancel-matches" onClick={this.handleFullProfileClose}>
            {strings.closeButton}
          </Button>
        </Modal.Actions>
      </Modal>
...
export default connect(mapStateToProps, {fetchTargetProfiles})(MatchingProfile);
