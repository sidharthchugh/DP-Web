...
import {fetchSearchSignup} from '../actions/searches';
import { addNotification } from 'actions/notification';
import ProfileHeader from '../components/Reusable/ProfileHeader';
import MatchSignupCard from '../components/Matches/MatchSignupCard';
...
class MatchesContainer extends React.Component {
  ...
  componentWillMount() {
   const searchId = this.props.params.searchId;
      this.props.fetchSearchSignup({searchId}).then(action =>
  ...
  renderMatches(sidebar) {
    ...
    return (
      <div>
        {!sidebar && (emailId.includes('digitalpartners.io') || emailId.includes('tractionb2b.com') || ENV === 'development') && (productSearchLoad.length || productDetailSearchLoad.length || partnerSearchLoad.length || partnerDetailSearchLoad.length || projectDetailSearchLoad.length || startupSearchLoad.length > 0) && refreshMatch() &&
     ...
     }{!sidebar && (!emailId.includes('digitalpartners.io') && !emailId.includes('tractionb2b.com') && ENV === 'development') && (productSearchLoadUser.length || productDetailSearchLoadUser.length || partnerSearchLoadUser.length || partnerDetailSearchLoadUser.length || projectDetailSearchLoadUser.length || startupSearchLoadUser.length > 0) &&
       <Grid className="match-card--container card-generic match-noResultCard">
         <Grid.Row>
           <Grid.Column computer={12}>
             <div className="no-matchResult">
        ...
        {!(productSearch.length + productDetailSearch.length + partnerSearch.length + partnerDetailSearch.length + projectDetailSearch.length + startupSearch.length) < 1 &&
            matches.length > 0 && matches.sort((a, b) => b.search.meta.createdAt - a.search.meta.createdAt) && matches.map((match, i) =>
              <div>
                ...
                <div>
                  <MatchSignupCard key={'match-card-' + i} match={match} index={i} matchId={this.props.params.searchId} userLanguage={userLanguage} sidebar={sidebar} />
                </div>
                }
              </div>
            )
...
const Matches = connect(
  mapStateToProps,
  {fetchSearchSignup, fetchMatches, toggleMatchesLoaded, addNotification}
)(MatchesContainer);
